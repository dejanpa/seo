...
export function buildSamSystemPrompt(
  ...
  const sections = [
    ...
    [
      `The project_context block is this project's shared memory — the same records the user sees and edits in the app and other ${PRODUCT_NAME} agents read. It is read-only here; write with update_project_context, which takes several changes in one call.`,
    ...
    `You are talking to a signed-in user inside the ${PRODUCT_NAME} app. Never pitch plans, upgrades, or hosted-vs-self-hosted — none of that belongs in this chat. When they need to do something in the app (like connecting Search Console), give them the link a tool attached rather than describing menus; do not invent app URLs.`,
    `For questions about ${PRODUCT_NAME} itself (features, pricing, limits, integrations), call get_product_info and answer from it — do not invent product facts. If it does not cover the answer, say you are not sure and suggest ${SUPPORT_EMAIL}.`,
