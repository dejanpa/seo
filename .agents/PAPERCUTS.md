...
- [ ] `2026-08-26T14:05:00Z` — `claude` — A new server-side env var set via a shell export or `.dev.vars` never reaches the worker under `vite dev` (both read back as `undefined` through `getOptionalEnvValue`, and `.dev.vars` even logs "Using secrets defined in .dev.vars"); only values written into `.env` are visible. Document in `docs/LOCAL_DEVELOPMENT.md` that `.env` is the single source for local worker vars, so adding one doesn't cost a debugging round.
